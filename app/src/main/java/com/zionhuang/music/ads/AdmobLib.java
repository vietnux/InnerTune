...
public class AdmobLib implements Application.ActivityLifecycleCallbacks {
    ...
    private static final int TIMEOUT_SECONDS = 30000; // Thời gian chờ tối đa

    public static synchronized AdmobLib getInstance( Activity activity ) {
        if (INSTANCE == null) {
...
//    }
    ...
    private void init() {
        ...
        if (DEBUG) {
            List<String> testDeviceIds = List.of("E083AAA96698F0005C809A492F7FAB06");
...
//                rewardedLoad();
    ...
    public void loadAdWithTimeout(Runnable callback) {
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            if (DEBUG) Log.e(TAG, "Load ad timeout!");
            callback.run(); // Gọi function callback
        }, TIMEOUT_SECONDS);
    }
    public void interstitial(final boolean show, AlertDialog pDl, @Nullable RemoteJsonCallback callback ) {
        isAdLoaded = false;
        isAdClosed = false;
        if(  ( JsonParams.DATA == null || !Objects.equals(JsonParams.getParam("interstitial"), "admob") || JsonParams.getParam("admobid.android.interstitial") == null ) ) {
            ...
            if( activity.getClass().getSimpleName().equals("MainActivity")) {
                if (callback != null) callback.onError(new Exception("Data null"));
            }
            return;
        } else {
            if( pDl != null ) {
        ...
        loadAdWithTimeout( () -> {
            if (!isAdLoaded) {
                ...
                if( activity.getClass().getSimpleName().equals("MainActivity")) {
                    if (callback != null) callback.onError(new Exception("Time out"));
                }
                isAdClosed = true;
            }
        } );
...
//            InterstitialAd.load(activity, "ca-app-pub-3940256099942544/1033173712", adRequest, new InterstitialAdLoadCallback() {
            InterstitialAd.load(activity, Objects.requireNonNull(JsonParams.getParam("admobid.android.interstitial")), adRequest, new InterstitialAdLoadCallback() {
                @Override
...
//                        activity.startActivity(intent);
                    ...
                    if( activity.getClass().getSimpleName().equals("MainActivity")) {
                        if (callback != null) callback.onError(new Exception("Load fail"));
                    }
...
//            isAdClosed = true;
    ...
    public void interstitialShow( @Nullable RemoteJsonCallback callback) {
            ...
            mInterstitialAd.setFullScreenContentCallback(
                    new FullScreenContentCallback() {
                        ...
                        public void onAdFailedToShowFullScreenContent(AdError adError) {
                            ...
                            if( activity.getClass().getSimpleName().equals("MainActivity")) {
                                if (callback != null) callback.onError(new Exception("The ad failed to show"));
                            }
                            isAdClosed = true;
                        }
...
//            interstitial(true, null);
            ...
            if( activity.getClass().getSimpleName().equals("MainActivity")) {
                if (callback != null) callback.onError(new Exception("The interstitial wasn't loaded yet"));
            }
            isAdClosed = true;
        }
    }

    public void adsInterstitialRandom( @Nullable RemoteJsonCallback callback ) {
    ...
    public void nativeads(final int native_id, TemplateView nativeads_template) {
...
//                });
    ...
    public  void showAdIfAvailable ( @Nullable RemoteJsonCallback callback) {
...
//            LoadingDialog.dismissLoadingDialog();
            fetchAd(pDialog, callback);

        }
    }

    /** Request an ad */
    public void fetchAd( AlertDialog ppDialog, @Nullable RemoteJsonCallback callback ) {
        pDialog = ppDialog;
...
//            Log.e(TAG, " isAdAvailable() = " +isAdAvailable() );
            ...
            if( activity.getClass().getSimpleName().equals("MainActivity")) {
                if (callback != null) callback.onError(new Exception("Have unused ad, no need to fetch another"));
            }
            return;
        }
