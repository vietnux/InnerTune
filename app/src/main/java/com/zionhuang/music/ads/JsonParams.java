...
public class JsonParams {
    ...
    public static float numOfDayPass = 0;
...
//            Log.e("Lỗi", "Failed to get param "+param, e);
    ...
    public static int getParamInt( String param ){
        String params[] = param.split("\\.");
        try {
            if(param == "download" || param == "background" || param == "shared" || param == "showcomment") {
                if( numOfDayPass > 0 && (float)getParamInt("numOfDayPass")>0 && numOfDayPass>(float)getParamInt("numOfDayPass") ) {
                    return 1;
                }
            }
            if (params.length == 1) {
